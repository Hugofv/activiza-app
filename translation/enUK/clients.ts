export default {
  newClient: 'New client',
  name: 'Name',
  blacklist: 'Blacklist',
  nameQuestion: "What is your client's full name?",
  nameRequired: 'Name is required',
  nameMinLength: 'Name must be at least 2 characters',
  nameMaxLength: 'Name must be at most 120 characters',
  nameInvalidChars:
    "Name can only contain letters, spaces, hyphen (-), and apostrophe (')",
  nameFullRequired: 'Enter full name (first name and surname)',
  whatsapp: 'WhatsApp',
  whatsappQuestion: "What is {{name}}'s WhatsApp?",
  whatsappRequired: 'WhatsApp is required',
  email: 'Email',
  emailQuestion: "What is {{name}}'s email address?",
  emailInvalid: 'Invalid email',
  document: 'Document',
  documentQuestion: "What is {{name}}'s document?",
  address: 'Address',
  observation: 'Observation',
  observationQuestion: 'Any observations?',
  observationPlaceholder: 'Write something',
  observationDescription:
    'You can describe any characteristic or important data for identification or location',
  guarantor: 'Guarantor',
  guarantorQuestion: 'Guarantor',
  guarantorSelect: 'Select a contact',
  reliability: 'Reliability',
  reliabilityQuestion: 'Reliability',
  reliabilityLevel: 'Level {{level}}',
  listChipReliabilityA11y: 'Reliability: {{value}}',
  listChipLoansDueA11y: 'Loans due soon: {{value}}',
  listChipPaidContractsA11y: 'Paid-off contracts: {{value}}',
  listChipDebtA11y: 'Amount in debt: {{value}}',
  summary: 'Summary',
  summaryName: 'Name',
  summaryWhatsapp: 'WhatsApp',
  summaryEmail: 'Email address',
  summaryDocument: 'Document',
  summaryAddress: 'Physical address',
  summaryObservation: 'Observation',
  summaryGuarantor: 'Referral / Guarantor',
  summaryReliability: 'Reliability',
  summaryNotInformed: 'Not informed',
  summaryConfirm: 'Confirm and save',
  successTitle: 'Success',
  successMessage: 'Client created successfully!',
  errorTitle: 'Error',
  errorMessage: 'Could not create client. Please try again.',
  yourClient: 'your client',
  addressNumber: 'No. {{number}}',
  addressComplement: 'Apt {{complement}}',
  addressPostalCode: 'ZIP {{code}}',
  cancel: 'Cancel',
  cancelConfirmTitle: 'Cancel registration?',
  cancelConfirmMessage:
    'Are you sure you want to cancel? All entered data will be lost.',
  cancelConfirmButton: 'Yes, cancel',
  cancelKeepButton: 'Continue registering',
};
