export default {
  lendLoan: 'Empréstimo',
  lendLoanDescription: 'Registre um novo empréstimo para um cliente.',
  selectClient: 'Selecionar cliente',
  lendToNewClient: 'Emprestar para novo cliente',
  amount: 'Valor',
  interest: 'Taxa de juros (%)',
  dueDate: 'Data do empréstimo',
  observation: 'Observação',
  createLoan: 'Criar empréstimo',
  invalidValue: 'Valor inválido',
  next: 'Próximo',
  confirm: 'Confirmar',
  contractSummary: 'Resumo do contrato',
  client: 'Cliente',
  profit: 'Lucro',
  loanDate: 'Data do empréstimo',
  frequency: 'Frequência',
  nextPayment: 'Recebimento',
  daily: 'Diário',
  weekly: 'Semanal',
  biweekly: 'Quinzenal',
  monthly: 'Mensal',
  currency: 'Moeda',
  alerts: 'Avisos',
  today: 'Hoje',
  // Loan list
  loansList: 'Empréstimos',
  dailyContract: 'Contrato diário',
  weeklyContract: 'Contrato semanal',
  biweeklyContract: 'Contrato quinzenal',
  monthlyContract: 'Contrato mensal',
  closed: 'Encerrado',
  totalWithInterest: 'Total com juros',
  filterAll: 'Todos',
  filterActive: 'Ativos',
  filterCompleted: 'Encerrados',
  filterOverdue: 'Atrasados',
  noLoans: 'Nenhum empréstimo encontrado',
  loadError: 'Erro ao carregar empréstimos',
  loanScreenTitle: 'Empréstimo',
  contractTitleFallback: 'Contrato #{{id}}',
  inProgress: 'Em andamento',
  statusFinalized: 'Finalizado',
  statusOverdue: 'Atraso',
  statusCancelled: 'Cancelado',
  toReceive: 'A receber: {{amount}}',
  registerPayment: 'Registrar pagamento',
  minimum: 'Mínimo',
  total: 'Total',
  paymentRegistered: 'Pagamento registrado',
  paymentDate: 'Data do pagamento',
  finalizeContract: 'Finalizar contrato',
  paymentHistory: 'Histórico',
  operationHistory: 'Histórico da operação',
  historyPayment: 'Pagamento',
  historyEmpty: 'Nenhum evento no histórico ainda.',
  historyLoadError: 'Erro ao carregar histórico da operação.',
  historyReason: {
    CREATED: 'Contrato criado',
    UPDATED: 'Contrato atualizado',
    DELETED: 'Contrato excluído',
    PAYMENT: 'Pagamento',
    COMPLETED: 'Contrato encerrado',
    CONTRACT_EVALUATION: 'Avaliação do contrato',
    MANUAL: 'Ajuste manual',
    OTHER: 'Evento',
  },
  fullPayment: 'Pagamento completo',
  partialPayment: 'Pagamento parcial',
  loanHistoryEmpty: 'Nenhum pagamento registrado ainda.',
  paymentsLoadError: 'Erro ao carregar histórico de pagamentos.',
  loanLoadError: 'Não foi possível carregar o empréstimo.',
  featureComingSoon: 'Em breve',
  loanActionDelete: 'Apagar',
  interestMetric: 'Juros',
  // Vehicle rental
  vehicles: 'Veículos',
  vehicleRental: 'Aluguel de veículo',
  vehicleRentalDescription: 'Registre um novo aluguel de veículo.',
  newRental: 'Novo aluguel',
  selectVehicle: 'Selecionar veículo',
  rentalDate: 'Data do aluguel',
  annotations: 'Anotações',
  rented: 'Alugado',
  available: 'Disponível',
  filterAvailable: 'Disponíveis',
  filterRented: 'Alugados',
  noVehicles: 'Nenhum veículo encontrado',
  vehicleLoadError: 'Erro ao carregar veículos',
  edit: 'Editar',
  plate: 'Placa',
  mileage: 'Quilometragem',
  value: 'Valor',
  createRental: 'Criar aluguel',
  addVehicle: 'Adicionar veículo',
  vehicleType: 'Tipo do veículo',
  car: 'Carro',
  motorcycle: 'Moto',
  brand: 'Marca',
  vehicleModel: 'Modelo',
  vehicleYear: 'Ano',
  currentMileage: 'Quilometragem atual',
  optional: 'opcional',
  deleteItems: 'Apagar {{count}} itens',
  deleteVehiclesConfirm: 'Tem certeza que deseja apagar {{count}} veículo(s)?',
  // Vehicle creation flow
  vehiclePhotos: 'Fotos do veículo',
  addPhoto: 'Adicionar foto',
  anyObservation: 'Alguma observação?',
  writeSomething: 'Escreva algo',
  finish: 'Concluir',
  cancelVehicleTitle: 'Descartar veículo?',
  cancelVehicleMessage: 'Você perderá todos os dados inseridos.',
  keepEditing: 'Continuar editando',
  permissionRequired: 'Permissão necessária',
  galleryPermissionMessage: 'Precisamos de permissão para acessar suas fotos.',
  imagePickerError: 'Erro ao selecionar imagem',
  // Vehicle detail
  mileageHistory: 'Quilometragem',
  maintenance: 'Manutenções',
  inspections: 'Vistorias',
  photos: 'Fotos',
  noPhotos: 'Nenhuma foto',
  maintenanceHistory: 'Histórico de manutenção',
  maintenanceType: 'Tipo de manutenção',
  maintenanceDate: 'Data da manutenção',
  addFiscalReceipt: 'Adicionar cupom fiscal',
  fiscalReceipt: 'Cupom fiscal',
  mileageDate: 'Data da quilometragem',
  register: 'Registrar',
  configure: 'Configurar',
  noMaintenanceRecords: 'Nenhuma manutenção encontrada',
  noMileageRecords: 'Nenhum registro de quilometragem',
  maintenanceAlerts: 'Alertas',
  markAsDone: 'Marcar como feita',
  remindLater: 'Lembrar depois',
  maintenanceOptions: 'Opções',
  configureAlerts: 'Configurar alertas',
};
