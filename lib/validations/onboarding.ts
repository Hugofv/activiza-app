  ...
  return yup.object().shape({
    document: documentBase.test('document-format', function (value) {
      ...
      const typeToValidate =
        currentDocumentType !== undefined ? currentDocumentType : documentType;
      ...
      switch (countryCode) {
        ...
        case 'UK':
          ...
          return this.createError({
            message:
              t('documentFormat')() ||
              'Document must be in NI Number format (AB123456C) or CRN format (8 digits)',
          });
        ...
        default:
          ...
          if (value.length < 5 || value.length > 30) {
            return this.createError({
              message:
                t('documentFormat')() ||
                'Document must have between 5 and 30 characters',
            });
          }
          return true;
      }
